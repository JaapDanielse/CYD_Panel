...
void Graph::CYD_HandleMessage(CYD_param param, CYD_type type)
{
	if(_inUse)
	{
		...
		{
			if(_rolling && Panel.cyds_int <= MAX_GRAPH_VALUE_COUNT)	_valueCount = Panel.cyds_int; else _valueCount = MAX_GRAPH_VALUE_COUNT;
			if(!_rolling && Panel.cyds_int <= MAX_GRAPH_VALUE_COUNT / 2)	_valueCount = Panel.cyds_int; else _valueCount = MAX_GRAPH_VALUE_COUNT / 2;
		}

		if(param == CYD_VALUECOUNT && type == CYD_long)
		{
			if(_rolling && Panel.cyds_long <= MAX_GRAPH_VALUE_COUNT)	_valueCount = (int16_t) Panel.cyds_long; else _valueCount = MAX_GRAPH_VALUE_COUNT;
			if(!_rolling && Panel.cyds_long <= MAX_GRAPH_VALUE_COUNT / 2)	_valueCount = (int16_t) Panel.cyds_long; else _valueCount = MAX_GRAPH_VALUE_COUNT / 2;
		}
